    ...
    def translate_keywords(self, keywords):
        ...
        for keyword, count in keywords:
            try:
                # 翻译到英文
                try:
                    en_result = self.deepl_client.translate_text(keyword, target_lang="EN-US")
                    en_translation = en_result.text if en_result else keyword
                except Exception as e:
                    logger.warning(f"翻译到英文失败 '{keyword}': {str(e)}")
                    en_translation = keyword
                
                # 翻译到中文
                try:
                    zh_result = self.deepl_client.translate_text(keyword, target_lang="ZH")
                    zh_translation = zh_result.text if zh_result else keyword
                except Exception as e:
                    logger.warning(f"翻译到中文失败 '{keyword}': {str(e)}")
